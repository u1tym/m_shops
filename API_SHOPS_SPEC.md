...
- **定休日**: `is_closed: true` を設定（スロットは空でもよい）。必要なら `day_memo` に補足
- **祝日営業時間**: `holiday_hours` オブジェクトで別管理（構造は曜日と同様、`day_of_week` なし）
- 曜日に依存しないメモ（年末年始など）は店舗の `schedule_memo`
...
```json
{
  ...
  "memo": "祝日前日は短縮営業",
  "slots": [
    { "open_time": "11:00", "close_time": "15:00", "sort_order": 0 }
  ]
}
```
...
```json
{ "id": 50, "visit_date": "2026-06-15", "memo": "ランチ" }
```

一覧・サマリでは `last_visit_on`（最新の来店日）を返す。詳細では `visits[]` で全履歴。
...
一覧では子要素のうち **ジャンル** と **最寄り駅（簡略）** のみ含める。営業時間・メニュー・キーワード・画像一覧は詳細 API で取得する。

`has_image=true` を指定した場合のみ、先頭1枚分の `thumbnail`（`ImageMeta`、バイナリ除く）を各件に含める。参考画像のない店舗では `thumbnail` は `null`。件数の絞り込みには使わない。
...
| `has_image` | boolean | いいえ | 一覧での参考画像表示（`true`=表示 / `false` または未指定=非表示）。**検索件数には影響しない** |
...
- 複数パラメータ指定時は **AND 条件**
- `has_image=true` のとき、レスポンス各件に `thumbnail`（先頭1枚の `ImageMeta`、画像がある店のみ）を含める。バイナリは含めず、表示時は `GET /shops/{id}/images/{image_id}` で取得する
- `has_image=false` または未指定のとき、`thumbnail` は含めない（転送量削減）
- `has_image` は表示制御のみで、返却件数・絞り込み条件には影響しない
...
| `holiday_hours` | 店舗あたり 1 件で置換。`null` 送信で削除 |
| `menus` / `keywords` / `stations` / `visits` | 送信に含まれない既存 ID は論理削除。`id` 付きは更新、なしは新規 |
| `images` | §3.7 の置換方式 |
